...
import {
  ...
  Plane, Wallet, BadgeCheck, CircleDashed, TriangleAlert
} from "lucide-react";
...
async function getVisaData(slug) {
  const client = await clientPromise;
  const db = client.db('Eammu-Holidays');
  const doc = await db.collection('nigerianvisadata')
...
export async function generateMetadata({ params }) {
  ...
  const defaultTitle = `${countryName} Tourist Visa for Nigerian Nationals ${currentYear} — Requirements, Fees & How to Apply`;
  const defaultDesc  = `Complete ${currentYear} ${countryName} visa guide for Nigerian passport holders. Documents required, embassy fees in Nigeria, bank balance, photo size, processing time & expert tips. Apply from Lagos or Abuja.`;
  ...
  return {
    ...
    keywords:    d?.seo_and_metadata?.keywords?.join(", ") ||
      `${countryName} visa from Nigeria ${currentYear}, ${countryName} tourist visa Nigerian passport, ${countryName} visa requirements Nigeria, ${countryName} visa fee Nigeria, ${countryName} visa processing time Nigeria, ${countryName} visa documents Nigerian nationals, how to apply ${countryName} visa from Nigeria, ${countryName} visa bank balance Nigeria, ${countryName} visa photo size, ${countryName} embassy Nigeria Lagos Abuja`,
    alternates: {
      canonical: d?.seo_and_metadata?.canonical_url || `https://visaexpresshub.com/visa/nigeria/${cleanSlug}`,
    },
    openGraph: {
      title:       `${countryName} Tourist Visa for Nigerian Nationals — ${currentYear} Complete Guide`,
      description: `Embassy-verified ${countryName} visa checklist, processing time, fees & expert tips for Nigerian passport holders. Updated ${currentYear}.`,
      images:      [country?.flag || ""],
      type:        "article",
    },
    twitter: {
      card:        "summary_large_image",
      title:       `${countryName} Visa from Nigeria ${currentYear} — Nigerian Passport Guide`,
      ...
      <div className="space-y-1.5">
        {relatedNames.map((name, i) => {
          ...
          return (
            <Link
              key={i}
              href={`/visa/nigeria/${createSlug(name)}`}
              title={`${name} tourist visa for Nigerian passport holders`}
...
function Breadcrumb({ countryName, dark = false }) {
  ...
  return (
    <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs font-semibold mb-8 flex-wrap">
      ...
      <Link href="/visa/nigeria"       style={{ color: muted }} className="hover:text-white transition">Visa Guide — Nigerian Nationals</Link>
      <ChevronRight size={12} style={{ color: muted }} />
...
function FallbackVisaPage({ country, whatsappUrl, allCountries }) {
  ...
  const commonDocs = [
    "Valid Nigerian International Passport (minimum 6 months validity beyond travel dates, at least 2 blank visa pages)",
    "Old/previous Nigerian passports (if any) — submit all for complete travel/visa history",
    ...
    "Confirmed round-trip flight reservation (refundable or dummy ticket from Lagos or Abuja)",
    "Hotel booking / accommodation proof for entire duration of stay",
    "Nigerian bank statement — last 6 months (savings + current), healthy consistent balance",
    "Bank confirmation letter / manager's letter on bank letterhead",
    "Cover letter clearly stating purpose of travel, full itinerary, and confirmed return to Nigeria",
    "Travel insurance covering medical emergencies for the entire trip duration",
    "Proof of employment: appointment letter + last 3 salary slips OR business registration documents",
    "CAC certificate / business registration (for self-employed Nigerian applicants)",
  ];

  const faqs = [
    {
      q: `Do Nigerian nationals need a visa to visit ${countryName}?`,
      a: `Visa requirements depend on ${countryName}'s bilateral relationship with Nigeria. Most countries require a prior visa for Nigerian passport holders. A handful offer visa-on-arrival or e-Visa options. Contact the ${countryName} Embassy or High Commission in Nigeria, or reach our consultants for the latest ${currentYear} requirements.`,
    },
    {
      q: `How long does the ${countryName} visa take to process from Nigeria?`,
      a: `Standard processing typically takes 10–20 working days from Nigeria submission. Embassies are frequently busier for Nigerian applications. We always recommend applying at least 8–10 weeks before your travel date to be safe.`,
    },
    {
      q: `What bank balance is required for a ${countryName} visa for Nigerian nationals?`,
      a: `Most embassies expect a minimum equivalent of USD 3,000–8,000 (approx. NGN 4.5M–12M at current rates) for solo travelers, with 6 months of stable transaction history in a Nigerian bank. The exact amount depends on trip duration and the specific destination. Our consultants advise based on your individual financial profile.`,
    },
    {
      q: `Where do Nigerian nationals submit the ${countryName} visa application?`,
      a: `Depending on the destination country, you submit at the ${countryName} Embassy or High Commission in Abuja or Lagos, at a VFS Global Nigeria centre, or through an authorised visa application centre. Our team will guide you to the correct submission point in Nigeria.`,
    },
    {
      q: `What is the photo specification for ${countryName} visa for Nigerian passport holders?`,
      a: `Most embassies require 35×45mm photos on a plain white background, ICAO compliant, taken within 3 months. No glasses, no head coverings (unless for religious reasons), no heavy retouching. Some countries (e.g. USA) require 51×51mm (2×2 inch) photos.`,
    },
  ];
  ...
  const structuredData = {
    ...
    "@type": "Article",
    "headline": `${countryName} Tourist Visa for Nigerian Nationals ${currentYear} — Requirements & Documents`,
    "description": `Complete ${countryName} tourist visa guide for Nigerian passport holders — documents, bank balance, photo specs, processing time & expert tips.`,
    "image": country.flag,
    ...
    "breadcrumb": {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 2, "name": "Visa Guide — Nigerian Nationals","item": "https://visaexpresshub.com/visa/nigerian-nationals" },
        { "@type": "ListItem", "position": 3, "name": `${countryName} Visa`,           "item": `https://visaexpresshub.com/visa/nigerian-nationals/${cleanSlug}` },
      ],
    },
  };
    ...
    <div className="min-h-screen" style={{ background: "#fff", fontFamily: "'DM Sans',system-ui,sans-serif", color: "#111" }}>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <div className="flex flex-wrap gap-2 mb-6">
                ...
                <span className="tag tag-dark">Nigerian Passport Holders</span>
              ...
              <h1 className="font-display text-4xl md:text-5xl xl:text-6xl leading-tight font-black mb-3" style={{ color: "white" }}>
                {countryName} <span style={{ color: "#f5c800" }}>Tourist Visa</span> for Nigerian Nationals
              </h1>
              <h2 className="text-base font-semibold mb-4 leading-relaxed" style={{ color: "rgba(255,255,255,0.5)" }}>
                How to apply for {countryName} visa from Nigeria — {currentYear} complete requirements guide for Nigerian passport holders
              </h2>
              <p className="leading-relaxed text-sm mb-8 max-w-lg" style={{ color: "rgba(255,255,255,.55)" }}>
                Detailed {countryName} visa data for Nigerian nationals is being prepared. The general requirements below are a reliable starting point. Contact our consultants for Nigeria-specific guidance.
              </p>
              ...
              <WhatsAppBtn href={whatsappUrl} label={`Ask About ${countryName} Visa from Nigeria`} />
            </div>
            <div className="flex justify-center md:justify-end">
              <div className="relative">
                <div className="w-72 h-48 rounded-2xl overflow-hidden shadow-2xl" style={{ border: "3px solid rgba(245,200,0,.4)" }}>
                  <img src={country.flag} alt={`${countryName} flag — ${countryName} tourist visa from Nigeria`} className="w-full h-full object-cover" />
                </div>
      ...
      <div className="max-w-7xl mx-auto px-5 mt-8 mb-6">
        <div className="flex gap-4 items-start p-5 rounded-2xl" style={{ background: "#fff8d6", border: "1.5px solid rgba(245,200,0,.4)" }}>
          ...
          <div>
            <p className="font-black text-black text-sm mb-1">Detailed {countryName} visa data for Nigerian nationals coming soon</p>
            <p className="text-sm leading-relaxed" style={{ color: "#555" }}>
              Our team is verifying the latest {currentYear} requirements from the {countryName} Embassy or High Commission in Nigeria. The checklist below is a reliable general guide. WhatsApp our consultants — they respond within 2 hours.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <section className="card p-8 md:p-10">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="font-display text-2xl font-black text-black">{countryName} Visa from Nigeria — What You Need to Know</h2>
              </div>
              <div className="space-y-4 text-sm leading-relaxed" style={{ color: "#555" }}>
                <p>If you hold a <strong className="text-black">Nigerian passport</strong> and plan to visit <strong className="text-black">{countryName}</strong>, you will most likely need to apply for a visa in advance from Nigeria. The process involves submitting a document package to the {countryName} Embassy, High Commission or Consulate in Nigeria (Lagos or Abuja), or to an authorised Visa Application Centre (VFS Global / BLS International) in Nigeria.</p>
                <p>The <strong className="text-black">{countryName} visa for Nigerian nationals</strong> in {currentYear} generally requires proof of financial solvency from a Nigerian bank, a confirmed travel itinerary, hotel bookings, proof of strong ties to Nigeria (employment, business, family), and a well-structured cover letter. Embassy officers particularly scrutinise Nigerian bank statements and proof of intent to return.</p>
                <p>Common rejection reasons for Nigerian applicants: <strong className="text-black">insufficient or unstable Nigerian bank balance</strong>, weak cover letter with vague travel purpose, non-compliant photographs, inconsistent travel dates, missing employment or business proof, and poor documentation of ties to Nigeria.</p>
              </div>
            </section>
            ...
            <section className="card p-8 md:p-10">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  <h2 className="font-display text-2xl font-black text-black">{countryName} Visa Checklist — Nigerian Passport Holders</h2>
                  <p className="text-sm mt-0.5" style={{ color: "#777" }}>Standard checklist for Nigerian nationals applying for {countryName} visa</p>
                </div>
              </div>
              ...
              <div style={{ borderTop: "1.5px solid #eee", paddingTop: "28px" }}>
                <p style={{ fontSize: "10px", fontWeight: 800, textTransform: "uppercase", letterSpacing: ".15em", color: "#aaa", marginBottom: "20px" }}>Additional Documents by Profession (Nigerian Context)</p>
                <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { role: "Nigerian Salaried",    items: ["Employer introduction letter on company letterhead (signed by HR/MD)", "Last 3–6 months payslips", "Employment contract / appointment letter", "Staff ID card copy"] },
                    { role: "Nigerian Business",    items: ["CAC certificate of incorporation", "Business name registration (if applicable)", "Company bank statement (6 months)", "Tax clearance certificate (TCC)"] },
                    { role: "Student (Nigeria)",    items: ["University/school letter of admission or enrollment", "School ID card", "Parent/guardian's financial proof and bank statement", "Academic transcripts"] },
                    { role: "Retired / Pensioner",  items: ["Pension statement (PENCOM/PFA letter)", "Retirement letter from former employer", "Any investment/asset statements", "Guarantor letter if applicable"] },
                  ].map(({ role, items }) => (
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <section className="card-yellow p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="font-display text-xl font-black text-black">Approval Tips for Nigerian Nationals</h2>
                </div>
                <div className="space-y-3">
                  {[
                    "Apply at least 8–12 weeks before your travel date. Nigerian applications often experience longer embassy queues — especially for USA, UK, Canada, and Schengen visas.",
                    "Maintain a healthy, stable Nigerian bank balance for at least 6 months before applying. Avoid large cash deposits just before applying — embassies flag this as suspicious for Nigerian applicants.",
                    "Write a strong, detailed cover letter: your job, monthly income, clear travel purpose, full itinerary, and exactly why you will return to Nigeria (family, employment, business).",
                    "Prove strong ties to Nigeria — a Nigerian employer letter, CAC certificate, property ownership, or family responsibilities. Embassies assess immigration risk carefully for Nigerian nationals.",
                    "Include copies of prior visas (USA, UK, Schengen, UAE, etc.) if you have them — prior visa history dramatically boosts your approval chances as a Nigerian applicant.",
                  ].map((tip, i) => (
            ...
            <section className="card p-8 md:p-10">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="font-display text-2xl font-black text-black">Bank Balance Required — {countryName} Visa from Nigeria</h2>
                  <p className="text-sm mt-0.5" style={{ color: "#777" }}>General Nigerian bank balance guidelines for {countryName} visa applicants</p>
                </div>
              </div>
              <div className="grid sm:grid-cols-3 gap-4 mb-6">
                {[
                  { label: "Solo Traveler",  val: "NGN 4.5M – 8M",   note: "NGN 10M+ recommended",              bg: "#eff6ff", border: "#bfdbfe", col: "#2563eb" },
                  { label: "Couple Travel",  val: "NGN 8M – 15M",    note: "Stable 6-month history required",    bg: "#f0fdf4", border: "#bbf7d0", col: "#16a34a" },
                  { label: "Family Trip",    val: "NGN 15M – 30M+",  note: "Varies by trip duration & size",     bg: "#faf5ff", border: "#e9d5ff", col: "#9333ea" },
              ...
              <div className="flex gap-3 p-5 rounded-2xl" style={{ background: "#fff8d6", border: "1.5px solid rgba(245,200,0,.4)" }}>
                ...
                <p className="text-sm font-medium leading-relaxed" style={{ color: "#555" }}>
                  <strong className="text-black">Important:</strong> Do not deposit a large lump sum into your Nigerian bank account immediately before applying. Embassy officers check the full 6-month transaction history — a sudden balance spike is a major red flag for Nigerian applicants. Build your balance steadily over months.
                </p>
              </div>
            </section>
            ...
            <section className="card p-8 md:p-10">
              <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                  <h2 className="font-display text-2xl font-black text-black">FAQ — {countryName} Visa for Nigerian Nationals {currentYear}</h2>
                  <p className="text-sm mt-0.5" style={{ color: "#777" }}>Common questions from Nigerian passport holders applying for {countryName} visa</p>
                </div>
              </div>
              <div>
                {faqs.map((item, i) => (
            ...
            <section className="card p-8 md:p-10">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="font-display text-2xl font-black text-black">{countryName} Visa for Nigerian Nationals — {currentYear} Guide</h2>
              </div>
              <div className="space-y-5 text-sm leading-relaxed" style={{ color: "#555" }}>
                <p>Getting a <strong className="text-black">{countryName} visa from Nigeria</strong> in {currentYear} requires careful and thorough preparation. Nigerian passport holders are scrutinised carefully at most embassies — which means your document file must be complete, consistent, and professionally assembled.</p>
                <h3 className="text-lg font-black text-black">{countryName} Visa Application Process for Nigerian Nationals</h3>
                <p>The process begins at the {countryName} Embassy, High Commission or Consulate in Nigeria (Abuja or Lagos), or at a VFS Global / BLS International centre in Nigeria. You complete the visa application form, attach all required documents, pay the applicable embassy and service fees, and await the visa officer's decision. Biometrics (fingerprints and photo) are required for most Schengen, UK, USA, and Canadian applications.</p>
                <h3 className="text-lg font-black text-black">How Long Does {countryName} Visa Processing Take from Nigeria?</h3>
                <p>Standard processing typically takes <strong className="text-black">10–20 working days</strong> from Nigeria submission. We strongly advise applying at least 8–12 weeks before your flight, especially during peak travel seasons and Nigerian public holidays.</p>
                <h3 className="text-lg font-black text-black">Can You Help Nigerian Nationals with {countryName} Visa Applications?</h3>
                <p>Yes — our experienced consultants specialise in Nigerian passport applications. <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="font-bold" style={{ color: "#d4a800", textDecoration: "underline" }}>WhatsApp us</a> or email <strong className="text-black">info@visaexperthub.com</strong> to get started today.</p>
              </div>
            </section>
            ...
            <section className="card-yellow p-8 rounded-2xl">
              ...
              <p className="text-xs mb-6" style={{ color: "#888" }}>Popular tourist visa guides for Nigerian passport holders — updated 2026</p>
              <div className="grid sm:grid-cols-2 gap-2.5">
                {["United States", "United Kingdom", "Canada", "Germany", "Japan", "Australia", "Malaysia", "Thailand"].map(name =>
                  name !== countryName ? (
                    <Link key={name} href={`/visa/nigeria/${createSlug(name)}`}
                      title={`${name} tourist visa from Nigeria — complete guide`}
                      ...
                      {name} Tourist Visa — Nigeria Guide
                    </Link>
                  ) : null
                )}
              </div>
          ...
          <aside className="lg:col-span-4 space-y-5">
            ...
            <div className="card p-6">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="font-black text-black text-sm">How to Apply from Nigeria — 5 Steps</h3>
              </div>
              <div className="space-y-5 relative">
                <div className="step-line" />
                {[
                  { n: "01", title: "Check Requirements",  desc: `Verify ${countryName} visa requirements for Nigerian passport holders in ${new Date().getFullYear()}` },
                  { n: "02", title: "Collect Documents",   desc: "Gather all required Nigerian financial & employment papers using the checklist above" },
                  { n: "03", title: "Book Appointment",    desc: "Schedule at VFS Global Nigeria, or the embassy/consulate in Lagos or Abuja" },
                  { n: "04", title: "Submit & Pay Fees",   desc: "Submit in person at the Nigeria centre; all embassy fees are non-refundable" },
            ...
            <div className="rounded-2xl p-5" style={{ background: "#eff6ff", border: "1.5px solid #bfdbfe" }}>
              <h3 className="font-black text-black text-sm mb-3 flex items-center gap-2">
                <MapPin size={14} style={{ color: "#2563eb" }} /> Visa Application Centres in Nigeria
              </h3>
              <div className="space-y-1.5">
                {["Abuja — Central Business District", "Lagos — Victoria Island", "Lagos — Ikeja", "Port Harcourt", "Kano", "Ibadan"].map(city => (
              ...
              <p className="text-[10px] mt-3 font-medium" style={{ color: "#4b7db8" }}>Availability varies by destination country. Our team will confirm the correct Nigeria submission centre for you.</p>
            </div>
            ...
            <div className="rounded-2xl p-6 text-center" style={{ background: "#f5c800" }}>
              ...
              <p className="text-sm leading-relaxed mb-5" style={{ color: "rgba(0,0,0,.6)" }}>Our consultants specialise in {countryName} visa applications for Nigerian nationals — complete document review from Lagos or Abuja.</p>
      ...
      <div className="py-20 px-5 text-center" style={{ background: "#111111", color: "white" }}>
        <div className="max-w-2xl mx-auto">
          ...
          <h2 className="font-display text-3xl md:text-4xl font-black mb-3">Planning to Visit {countryName} from Nigeria?</h2>
          <p className="mb-10 leading-relaxed text-sm max-w-lg mx-auto" style={{ color: "rgba(255,255,255,.5)" }}>
            Let our expert consultants guide your {countryName} visa application from Nigeria — the right documents, the right way, with a 98% approval rate for Nigerian passport holders.
          </p>
...
export default async function CountryVisaPage({ params }) {
  ...
  if (!country) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-10"
        ...
        <Link href="/visa/nigeria" className="btn-yellow px-8 py-4 rounded-2xl text-sm inline-flex">
  ...
  const whatsappMsg = encodeURIComponent(`Hi, I'm a Nigerian national and want to apply for a ${countryName} Tourist Visa. I found the guide on your website.`);
  ...
  const structuredData = {
    "@context": "https://schema.org",
    "@type": "Article",
    "headline": `${countryName} Tourist Visa for Nigerian Nationals ${currentYear} — Requirements, Fees & Documents`,
    "description": d.description || `Complete ${countryName} visa guide for Nigerian passport holders — updated ${currentYear}.`,
    "image": country.flag,
    "author": { "@type": "Organization", "name": "Visa Expert Hub" },
    "publisher": { "@type": "Organization", "name": "Visa Expert Hub", "logo": { "@type": "ImageObject", "url": "/logo.png" } },
    "dateModified": new Date().toISOString(),
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div>
              <div className="flex flex-wrap gap-2 mb-5">
                ...
                <span className="tag" style={{ background: "rgba(0,128,0,.2)", color: "#4ade80", border: "1.5px solid rgba(0,128,0,.3)" }}>🇳🇬 Nigerian Nationals</span>
              </div>
              <h1 className="font-display text-4xl md:text-5xl xl:text-6xl leading-tight font-black mb-3" style={{ color: "white" }}>
                {countryName} Tourist Visa <span style={{ color: "#f5c800" }}>for Nigerian Nationals</span>
              </h1>
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {["✅ 98% Success Rate", "📋 Embassy Verified", "⚡ 24hr Review", "🇳🇬 Nigeria Specialist", "🔒 Confidential"].map(b => (
                  <span key={b} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold"
            ...
            <div className="flex justify-center md:justify-end">
              <div className="relative">
                <div className="w-72 h-48 rounded-2xl overflow-hidden shadow-2xl" style={{ border: "3px solid rgba(245,200,0,.5)" }}>
                  <img src={country.flag} alt={`${countryName} flag — tourist visa for Nigerian nationals ${currentYear}`} className="w-full h-full object-cover" />
                </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6">
            ...
            <section className="card p-8 md:p-10">
              <div className="flex items-center gap-3 mb-6">
                <span className="section-line" />
                <h2 className="font-display text-2xl font-black text-black">{countryName} Visa for Nigerian Nationals — Everything You Need</h2>
              </div>
              <div className="space-y-4 text-sm leading-relaxed" style={{ color: "#555" }}>
                <p>For <strong className="text-black">Nigerian passport holders</strong>, obtaining a <strong className="text-black">{countryName} tourist visa</strong> requires meticulous document preparation, a strong Nigerian financial profile, and a professionally assembled application package submitted to the {countryName} Embassy or High Commission in Nigeria, or a designated VFS Global / BLS centre in Lagos or Abuja.</p>
                <p>The <strong className="text-black">{d.visa_category_details?.visa_type}</strong> is processed in <strong className="text-black">{d.processing_time_metrics?.standard_turnaround}</strong>. During busy periods this can extend to <strong className="text-black">{d.processing_time_metrics?.peak_season_delay}</strong>. We strongly recommend Nigerian nationals apply at least 8–12 weeks before the travel date.</p>
                <p>Most common rejection reasons for Nigerian applicants: <strong className="text-black">insufficient or inconsistent Nigerian bank balance</strong>, weak cover letter lacking clear ties to Nigeria, non-compliant photographs, missing CAC or employer documentation, and no prior international travel history.</p>
              </div>
            </section>
            ...
            <section className="card p-8 md:p-10">
              <div className="flex items-center gap-4 mb-8">
                <div className="p-3 rounded-2xl" style={{ background: "#f0fdf4" }}><CheckCircle size={24} style={{ color: "#16a34a" }} /></div>
                <div>
                  <h2 className="font-display text-2xl font-black text-black">{countryName} Visa — Mandatory Documents for Nigerian Nationals</h2>
              ...
              {d.comprehensive_requirements_checklist?.occupation_specific && (
                <div style={{ borderTop: "1.5px solid #eee", paddingTop: "28px" }}>
                  <p style={{ fontSize: "10px", fontWeight: 800, textTransform: "uppercase", letterSpacing: ".15em", color: "#aaa", marginBottom: "20px" }}>04. Occupation-Specific Documents (Nigerian Context)</p>
                  <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {d.geospatial_submission_data?.length > 0 && (
              <section className="card p-8 md:p-10">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <div>
                    <h2 className="font-display text-2xl font-black text-black">Official Submission Centres in Nigeria</h2>
                    <p className="text-sm mt-0.5" style={{ color: "#777" }}>Where to submit your {countryName} visa application from Lagos or Abuja</p>
                  </div>
                </div>
                <div className="grid md:grid-cols-2 gap-5">
            ...
            <section className="card-yellow p-8 md:p-10 rounded-2xl">
              <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                  <h2 className="font-display text-2xl font-black text-black">{countryName} Visa Approval Tips for Nigerian Nationals</h2>
                  <p className="text-sm mt-0.5" style={{ color: "#888" }}>Insider tips from our consultants — what embassies look for from Nigerian applicants</p>
                </div>
              </div>
            ...
            <section className="card p-8 md:p-10">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="font-display text-2xl font-black text-black">{countryName} Visa Rejection Risks — Nigerian Nationals</h2>
                  <p className="text-sm mt-0.5" style={{ color: "#777" }}>Why Nigerian applicants get rejected — and how to avoid it</p>
                </div>
              </div>
            ...
            {d.faq_extended?.length > 0 && (
              <section className="card p-8 md:p-10">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <div>
                    <h2 className="font-display text-2xl font-black text-black">FAQ — {countryName} Visa for Nigerian Nationals {currentYear}</h2>
                    <p className="text-sm mt-0.5" style={{ color: "#777" }}>Real questions from Nigerian passport holders applying for {countryName} visa</p>
                  </div>
                </div>
                <div>
            ...
            <section className="card p-8 md:p-10">
              <div className="flex items-center gap-3 mb-6">
                <span className="section-line" />
                <h2 className="font-display text-2xl font-black text-black">{countryName} Visa for Nigerian Nationals — {currentYear} Complete Guide</h2>
              </div>
              <div className="space-y-5 text-sm leading-relaxed" style={{ color: "#555" }}>
                <p>Applying for a <strong className="text-black">{countryName} tourist visa from Nigeria</strong> involves submitting a complete Nigerian document package to the {countryName} Embassy or VFS Global / BLS centre in Nigeria. The {d.visa_category_details?.visa_type} allows a stay of <strong className="text-black">{d.stay_and_validity_rules?.standard_stay}</strong>.</p>
                <h3 className="text-lg font-black text-black">{countryName} Visa Fee for Nigerian Nationals — {currentYear}</h3>
                <p>The current embassy fee is <strong className="text-black">{d.visa_fee_structure_2026?.embassy_visa_fee}</strong>. VFS Global Nigeria service charge adds <strong className="text-black">{d.visa_fee_structure_2026?.vfs_service_charge}</strong>. All fees are <strong className="text-black">non-refundable</strong> regardless of outcome.</p>
                <h3 className="text-lg font-black text-black">Bank Balance Required for {countryName} Visa from Nigeria</h3>
                <p>Nigerian applicants should maintain a minimum bank balance of <strong className="text-black">{d.financial_thresholds_2026?.solo_traveler_min}</strong> for solo travel, with <strong className="text-black">{d.financial_thresholds_2026?.solo_traveler_recommended}</strong> recommended. Avoid large last-minute Nigerian bank deposits.</p>
                <h3 className="text-lg font-black text-black">{countryName} Visa Photo Size for Nigerian Nationals</h3>
                <p>Photos must be <strong className="text-black">{d.technical_photo_specifications?.dimensions}</strong> ({d.technical_photo_specifications?.standard}), on a <strong className="text-black">{d.technical_photo_specifications?.background_color}</strong> background, taken within 3 months. Available at photo studios across Lagos, Abuja, and all major Nigerian cities.</p>
              </div>
            </section>
            ...
            <section className="card-yellow p-8 rounded-2xl">
              ...
              <div className="grid sm:grid-cols-2 gap-2.5">
                {["United States", "United Kingdom", "Canada", "Germany", "France", "Japan", "Australia", "Malaysia", "Thailand", "Singapore"].map(name =>
                  name !== countryName ? (
                    <Link key={name} href={`/visa/nigeria/${createSlug(name)}`}
                      title={`${name} tourist visa from Nigeria — complete 2026 guide`}
                      className="flex items-center gap-2 p-3 rounded-xl text-sm font-semibold transition-all group"
                      style={{ background: "rgba(255,255,255,.7)", border: "1.5px solid rgba(245,200,0,.2)", color: "#333" }}>
                      <span className="font-black group-hover:translate-x-0.5 transition-transform" style={{ color: "#d4a800" }}>→</span>
                      {name} Tourist Visa — Nigeria Guide
          ...
          <aside className="lg:col-span-4 space-y-5">
            ...
            <div className="rounded-2xl p-6" style={{ background: "#fff8d6", border: "1.5px solid rgba(245,200,0,.4)" }}>
              ...
              <div className="space-y-2 mb-5">
                ...
                <div className="flex justify-between items-center py-3" style={{ borderBottom: "1.5px solid rgba(245,200,0,.2)" }}>
                  <span className="text-sm font-semibold" style={{ color: "#555" }}>VFS Nigeria Service</span>
            ...
            <div className="card p-6">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="font-black text-black text-sm">Processing Timeline from Nigeria</h3>
              </div>
              <div className="space-y-5 relative">
                <div className="step-line" />
                {[
            ...
            <div className="rounded-2xl p-6 text-white" style={{ background: "#111111" }}>
              <div className="flex items-center gap-2 mb-4">
                <CreditCard size={17} />
                <h3 className="font-black text-sm">Minimum Nigerian Bank Balance</h3>
              </div>
            ...
            <div className="rounded-2xl p-6 text-center" style={{ background: "#f5c800" }}>
              ...
              <p className="text-sm leading-relaxed mb-5" style={{ color: "rgba(0,0,0,.6)" }}>
                Our experts handle form-filling, photo verification &amp; document review for Nigerian nationals applying for {countryName} visa.
              </p>
      ...
      <div className="py-20 px-5 text-center" style={{ background: "#111111" }}>
        <div className="max-w-2xl mx-auto">
          ...
          <h2 className="font-display text-3xl md:text-4xl font-black mb-3" style={{ color: "white" }}>
            Ready to Apply for Your <span style={{ color: "#f5c800" }}>{countryName} Visa as a Nigerian National?</span>
          </h2>
          <p className="mb-10 leading-relaxed text-sm max-w-lg mx-auto" style={{ color: "rgba(255,255,255,.45)" }}>
            Let our experts handle your complete {countryName} visa application from Nigeria — correctly, on time, with a 98% success rate for Nigerian passport holders.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
